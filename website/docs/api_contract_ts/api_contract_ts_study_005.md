...
![Public API Entry](./picture/api_contract_ts_study_005_internal_vs_public.png)
...
```mermaid
graph TD
    User[利用者] -->|import| Public[public-api.ts<br>公開入口]
    Public -->|export| Core[internal/calcCore.ts<br>内部実装]
    Public -->|export| Util[internal/parse.ts<br>内部実装]
    
    User -.->|直接アクセス禁止❌| Core
    style Public fill:#bbf,stroke:#333,stroke-width:2px
```
