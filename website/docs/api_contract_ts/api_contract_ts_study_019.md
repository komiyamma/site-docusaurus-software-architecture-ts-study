...
![Runtime Validation Shield](./picture/api_contract_ts_study_019_runtime_shield.png)
...
```mermaid
flowchart LR
    Data["外部データ<br>(unknown)"] --> Gate{"境界バリデーション<br>(Zod)"}
    Gate -->|OK| Internal["内部: 信頼できる型<br>(Typed)"]
    Gate -->|NG| Error["エラー応答<br>(400 Bad Request)"]
    
    style Internal fill:#dcf,stroke:#333
    style Data fill:#ddd,stroke:#999
```
...
![Zod as Quality Inspector](./picture/api_contract_ts_study_019_zod_gate.png)
