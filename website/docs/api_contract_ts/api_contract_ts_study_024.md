...
```mermaid
flowchart TD
    Start[バージョン分け必要？] --> Q1{"公開API？<br>利用者が多い？"}
    Q1 -->|Yes| URI["URI版 /v1<br>(標準的・分かりやすい)"]
    Q1 -->|No| Q2{"URLを変えたくない？<br>Gatewayで分けたい？"}
    Q2 -->|Yes| Header["Header版<br>api-version: 1"]
    Q2 -->|No| Q3{表現として厳密に分けたい？}
    Q3 -->|Yes| Accept["Accept版<br>vnd.example.v1+json"]
    Q3 -->|No| Query["Query版<br>?version=1"]
```
