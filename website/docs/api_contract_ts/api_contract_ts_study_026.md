...
## 26.2 イベントは「過去の事実」：名前の付け方が超大事📣⌛

![Events as Past Facts](./picture/api_contract_ts_study_026_event_facts.png)

イベント（Event）は「起きたこと（事実）」を伝えるもの。
...
![Event Envelope and Payload](./picture/api_contract_ts_study_026_event_envelope.png)
...
```mermaid
graph TD
    subgraph Envelope[封筒 standard fields]
        Meta[id, source, type, time]
        subgraph Payload[中身 detail]
            Data[data: UserSignedUp]
        end
    end
    style Envelope fill:#eee,stroke:#333
    style Payload fill:#ccf,stroke:#333
```
