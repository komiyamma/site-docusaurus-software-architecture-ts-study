...
![Pact Cycle Flow](./picture/api_contract_ts_study_030_pact_cycle_flow.png)
...
```mermaid
sequenceDiagram
    participant C as Consumer (利用者)
    participant B as Pact Broker
    participant P as Provider (提供者)
    ...
    Note over P: 3. 検証実行
    P->>B: Fetch Pact
    B-->>P: Contract definitions
    P->>P: Verify with API ✅
    P->>B: Publish Result (Pass/Fail)
```
...
![Can I Deploy? Gate](./picture/api_contract_ts_study_030_can_i_deploy.png)
