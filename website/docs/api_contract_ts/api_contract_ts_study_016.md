...
![Error Bins](./picture/api_contract_ts_study_016_error_bins.png)
...
```mermaid
flowchart LR
    Lib[外部ライブラリ] -->|Throw Raw Error| Boundary[境界: 翻訳関数]
    Boundary -->|Convert| AppError["整形済みエラー<br>(Infra/Input/Domain)"]
    AppError -->|Return/Throw| User[利用者/UI]
```
