...
```mermaid
graph LR
    Provider[提供者<br>プログラムを作る人] -->|約束を守る| Contract[契約<br>インターフェース/ルール]
    User[利用者<br>プログラムを使う人] -->|約束を信じる| Contract
    Contract -->|安心| User
```
...
![Contract Locations](./picture/api_contract_ts_study_001_contract_locations.png)
...
![Implementation vs Contract](./picture/api_contract_ts_study_001_contract_door.png)
...
```mermaid
flowchart TD
    subgraph Contract[契約（公開されているもの）]
        Type[型: User]
        Func[関数名: fetchUser]
        Args[引数: id]
        Ret[戻り値: Promise<User>]
    end
    Impl[実装（中身のコード）] -.->|守る| Contract
    User[利用者] -->|頼る| Contract
```
