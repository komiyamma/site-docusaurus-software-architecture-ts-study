...
![Compatibility Directions](./picture/api_contract_ts_study_006_compat_directions.png)
...
![Tolerant Reader](./picture/api_contract_ts_study_006_tolerant_reader.png)
...
```mermaid
sequenceDiagram
    participant OldClient as 旧クライアント
    participant NewServer as 新サーバー
    
    OldClient->>NewServer: データをリクエスト
    NewServer-->>OldClient: 新しいデータ (id, name, nickname)
    Note right of OldClient: nicknameを知らない
    OldClient->>OldClient: id, nameだけ使う
    Note right of OldClient: 壊れない！ (Tolerant Reader)
```
