...
![Min Test Pyramid](./picture/api_contract_ts_study_029_test_pyramid_small.png)
...
```mermaid
graph BT
    Unit[ユニットテスト<br>・挙動/意味<br>・エラー形式] --> Type[型チェック tsc<br>・公開API<br>・引数/戻り値の形]
    TypeTest[型テスト<br>・推論/Generics<br>・意図の固定] -.-> Type
    
    style Type fill:#bbf
    style Unit fill:#bfb
    style TypeTest fill:#eee,stroke-dasharray: 5 5
```
