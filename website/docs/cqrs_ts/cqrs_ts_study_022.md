...
![cqrs_ts_study_022_result_pattern.png](./picture/cqrs_ts_study_022_result_pattern.png)

```mermaid
flowchart TD
  subgraph Result_Flow ["Result Flow (仕様)"]
    direction LR
    H[Handler] --> R{Result}
    R -- OK --> S[Success Response]
    R -- ERR --> E[Error Response]
  end
  subgraph Exception_Flow ["Exception Flow (事故)"]
    direction LR
    H2[Handler] -- throw --> G[Global Error Handler]
    G --> E2[500 Internal Server Error]
  end
```
