...
flowchart TD
  subgraph Transaction [Transaction Boundary]
    C[Command] --> O[Update Order]
    C --> M[Save Outbox Message]
  end
  M -- polling / tailing --> R[Outbox Relay]
  R -- publish --> EB[Event Bus / Queue]
  EB -- consume --> P[Projector]
  P -- update --> RM[Read Model]
```
