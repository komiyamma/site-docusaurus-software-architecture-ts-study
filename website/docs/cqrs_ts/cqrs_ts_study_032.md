...
![cqrs_ts_study_032_search_index.png](./picture/cqrs_ts_study_032_search_index.png)

```mermaid
flowchart LR
  subgraph Slow ["Full Scan (No Index)"]
    direction TB
    Q1["Filter: status='PAID'"] --> S1["Scan ALL Rows"]
    S1 --> R1[Result]
  end
  subgraph Fast ["Indexed Search"]
    direction TB
    Q2[Filter: status='PAID'] --> I1{Index Map}
    I1 -- 'PAID' --> P1[Pointer to Rows]
    P1 --> R2[Result]
  end
```
