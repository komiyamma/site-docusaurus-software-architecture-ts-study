...
## 35.1 まず結論：画面更新の3択🎯✨
![cqrs_ts_study_035_frontend_updates.png](./picture/cqrs_ts_study_035_frontend_updates.png)

```mermaid
flowchart TD
  S[Command Success] --> C{Choice}
  C -- Simple --> R[Re-fetch
  Simple but Laggy]
  C -- Fast Feel --> O[Optimistic Update
  UI first, Sync later]
  C -- Async UI --> N[Subscription / Push
  Wait for Projection]
```
