...
```mermaid
flowchart TD
  E[Event Source] -- publish --> B[Event Bus]
  B -- subscribe --> H1[Handler: List Projection]
  B -- subscribe --> H2[Handler: Sales Aggr]
  H1 --> R1[(Read Model: List)]
  H2 --> R2[(Read Model: Summary)]
```
