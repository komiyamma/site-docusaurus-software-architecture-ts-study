...
![cqrs_ts_study_030_idempotency.png](./picture/cqrs_ts_study_030_idempotency.png)

```mermaid
flowchart TD
  E[Event Received] --> G{Already Processed?}
  G -- Yes --> S[Skip / Success]
  G -- No --> D[Start Transaction]
  D --> P[Execute Projection]
  P --> R[Record Event ID]
  R --> C[Commit Transaction]
  C --> S
```
