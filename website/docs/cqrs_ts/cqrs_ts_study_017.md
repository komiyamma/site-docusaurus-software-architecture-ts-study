...
![cqrs_ts_study_017_aggregation.png](./picture/cqrs_ts_study_017_aggregation.png)

```mermaid
mindmap
  root((Sales Summary))
    Total Sales (Sum of totalYen)
    Order Count (Count of PAID)
    Average Value (Total / Count)
    Daily Sales (Group by paidDay)
    Top Menus (Sum quantity by menuId)
```
