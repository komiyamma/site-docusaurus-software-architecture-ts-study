...
![✅ ざっくり完成イメージが頭に入る👀✨](./picture/invariants_ts_study_001_flow.png)
...
```mermaid
flowchart TD
  Input["unknown: 外部の不確かな入力 🌍"] --> DTO["DTO: 型定義された箱 📦"]
  DTO -- "① 検証 (parse/safeParse)" --> Validate["検証済み状態 ✅"]
  Validate -- "② 正規化 (trim/lower)" --> Norm["整えられた値 🧼"]
  Norm -- "③ 値オブジェクトへ変換" --> VO["ドメイン型 (Result&lt;VO&gt;) 💎"]
  VO --> Domain["ドメイン内部: 安心な世界 🏰"]
```
...
![① 「境界で検証して変換する」🚧✅](./picture/invariants_ts_study_001_intro_check.png)
