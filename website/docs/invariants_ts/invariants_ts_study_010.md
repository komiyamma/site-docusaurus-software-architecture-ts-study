...
```mermaid
flowchart LR
  subgraph SameStorage ["メモリ上の実体は string"]
    U["'user_123'"]
    P["'prod_456'"]
  end
  
  subgraph TypeScript ["型システム上のブランド 🏷️"]
    UserId["UserId (string & {__brand})"]
    ProductId["ProductId (string & {__brand})"]
  end
