...
![第6章：「型で守れること」と「実行時に守ること」分けよう 🧩✅](./picture/invariants_ts_study_006_two_stage.png)
...
```mermaid
flowchart TD
  Input["外部入力 (unknown) 🕵️‍♀️"]
  subgraph Layer1 ["層1: 境界 (実行時検証) 🧪"]
    Check{検証 / 変換}
  end
  subgraph Layer2 ["層2: ドメイン (型で保護) 🏰"]
    Safe["検証済み型 (UserDTOなど) 🔒"]
    Logic["ビジネスロジック (if削減) ✨"]
  end
  Input --> Check
  Check -- 成功 --> Safe
  Safe --> Logic
  Check -- 失敗 --> Error["エラー返却 🔁"]
```
