...
![第2章　不変条件ってなに？（超やさしく）💎😊](./picture/invariants_ts_study_002_invariant.png)
...
```mermaid
flowchart LR
  subgraph Invariants ["不変条件 (Invariant)"]
    direction TB
    I1["金額 >= 0"]
    I2["在庫 >= 0"]
    I3["IDの不一致禁止"]
  end
  subgraph SimpleChecks ["ただのチェック"]
    direction TB
    C1["ニックネーム 20文字以内"]
    C2["アイコン画像 2MB以内"]
    C3["ボタンの色変更など"]
  end
  Invariants --- SimpleChecks
  style Invariants fill:#fdf,stroke:#f6f
```
