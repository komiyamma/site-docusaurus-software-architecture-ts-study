...
![第8章　型の武器①：リテラル型・ユニオン型で選択肢を固定🎫✨](./picture/invariants_ts_study_008_literal_type.png)
...
```mermaid
flowchart TD
  Input["入力: string 🖊️"] --> Case{リテラル型 / ユニオン型}
  Case -- "Free" --> OK1["プラン適用 (Free)"]
  Case -- "Pro" --> OK2["プラン適用 (Pro)"]
  Case -- "Fre (ミス)" --> NG["コンパイルエラー ⚡"]
  
  subgraph Protection ["選択肢の固定 🎫"]
    Case
  end
```
