...
flowchart LR
  ...
  subgraph Boundary ["境界 (門番) 🚧"]
    direction TB
    V[検証] --> N[正規化] --> C[変換]
  end
  
  subgraph Inner ["ドメイン (核心) 🏰"]
    Core[ビジネスの約束守る]
  end
...
![Step2: 入力→処理→出力を1本線で描く🧵✨](./picture/invariants_ts_study_004_boundary_steps.png)
...
![5-1. 境界関数は「外→中」の門にする🚪🛡️](./picture/invariants_ts_study_004_boundary_fn.png)
