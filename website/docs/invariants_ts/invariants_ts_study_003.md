...
![第3章　なぜバグる？チェックが散ると事故る😱🌀](./picture/invariants_ts_study_003_gate_guard.png)
...
![チェックが散って “漏れ” が出るコード](./picture/invariants_ts_study_003_scattered_checks.png)
...
```mermaid
flowchart TD
  subgraph Bad ["悪い例: チェックが散乱 😵‍💫"]
    UI1[UI] -- if --> Process1[処理]
    API1[API] -- if --> Process1
    DB1[DB] -- if --> Process1
  end
  subgraph Good ["良い例: 入口に集約 🛡️"]
    UI2[UI] --> Gate[門番: 境界 警察 🚓]
    API2[API] --> Gate
    DB2[DB] --> Gate
    Gate -- OK --> Castle[ドメイン: 信頼 🏰]
  end
```
