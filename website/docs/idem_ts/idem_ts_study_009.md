...
![Concept](./picture/idem_ts_study_009_contract.png)
...
sequenceDiagram
    ...
    C->>S: POST /orders (Key: uuid-123)
    S->>DB: Keyがあれば結果取得
    DB-->>S: なし
    S->>S: 処理実行 (注文作成)
    S->>DB: Keyと結果(201, Body)を保存
    S-->>C: 201 Created (o_999)
    ...
    S->>DB: Keyがあれば結果取得
    DB-->>S: 201, Body あり
    S-->>C: 201 Created (o_999) <br/> (保存済み結果を返却)
```
