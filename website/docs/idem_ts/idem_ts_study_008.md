...
![Concept](./picture/idem_ts_study_008_post_risk.png)
...
sequenceDiagram
    ...
    C->>S: POST /orders (1回目)
    S->>S: 注文#1 を作成 ✅
    Note right of S: レスポンス送信...
    Note over C,S: 通信障害で届かない ❌
    
    C->>S: POST /orders (2回目: リトライ)
    S->>S: 注文#2 を作成 ✅ (重複!)
    S-->>C: 201 Created (注文#2)
    
    Note over C,S: クライアントは1回成功したつもりだが、<br/>サーバーには2つの注文がある 😱
```
