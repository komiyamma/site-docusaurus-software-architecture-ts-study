...
![Concept](./picture/idem_ts_study_002_network.png)
...
sequenceDiagram
    ...
    S->>DB: 注文レコード作成 (注文#123)
    DB-->>S: OK
    Note over S,C: ネットワーク遅延 / 切断
    C-xS: レスポンスが届かない (タイムアウト)
    
    C->>S: 再送 POST /orders
    S->>DB: 注文レコード作成 (注文#124)
    DB-->>S: OK
    S-->>C: 201 Created (注文#124)
    
    Note over C,DB: データベースに 123 と 124 の二重注文が残る!
```
