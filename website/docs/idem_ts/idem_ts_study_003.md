...
![Concept](./picture/idem_ts_study_003_terms.png)
...
```mermaid
flowchart TD
    Retry[リトライ: 行動] --> Condition{再実行安全?}
    Condition -->|Yes| OK[安全に再試行完了]
    Condition -->|No| Fail[二重実行事故 💥]

    subgraph Safety[再実行安全にする手段]
        Idem[冪等性: 性質<br/>何回やっても同じ結果]
        Dedup[重複排除: 仕組み<br/>2回目を落とす]
    end
