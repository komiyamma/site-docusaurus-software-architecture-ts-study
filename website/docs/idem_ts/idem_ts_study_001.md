...
![Concept](./picture/idem_ts_study_001_concept.png)
...
```mermaid
graph TD
  Start[likes = 0] --> Op1[addLike 実行]
  Op1 --> Result1[likes = 1]
  Result1 --> Op2[addLike 実行]
  Op2 --> Result2[likes = 2]
  Result2 --> Fail{結果が変わった!}
  Fail -->|非冪等| End[連打すると壊れる]
```
...
```mermaid
graph TD
  Start[isSubscribed = false] --> Op1[setSubscribedTrue 実行]
  Op1 --> Result1[isSubscribed = true]
  Result1 --> Op2[setSubscribedTrue 実行]
  Op2 --> Result2[isSubscribed = true]
  Result2 --> Success{結果が同じ!}
  Success -->|冪等| End[連打しても壊れない]
```
