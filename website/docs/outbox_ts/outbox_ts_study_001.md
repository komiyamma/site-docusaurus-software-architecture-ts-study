...
graph TD
  User[ユーザー/アプリ] --> DB[(DB)]
  User -.->|失敗の可能性| Broker[メッセージブローカー]
  
  subgraph 事故パターン
    DB -- "✅ 成功" --> S1[注文確定]
    Broker -- "❌ 失敗" --> E1[イベント未送信]
  end
  
  style E1 fill:#f96,stroke:#333
```

![dual write](./picture/outbox_ts_study_001_dual_write.png)
* DBは成功✅
* イベント送信は失敗❌
* あるいはその逆もある😇
...
```mermaid
flowchart LR
    App[アプリ/サービス] --> Trans{トランザクション}
    subgraph DB [同一DB内]
        Trans --> Biz[業務テーブル<br/>注文など]
        Trans --> OB[Outboxテーブル<br/>送る予定]
    end
    DB --> Relay[送信係/CDC]
    Relay --> Broker[ブローカー]
```
...
sequenceDiagram
    ...
    participant DB as 業務DB & Outbox
    participant Relay as 送信係 (Relay/CDC)
    participant Broker as ブローカー

    Note over App, DB: ✅ ステップ1: トランザクション
    App->>DB: 注文保存 + Outbox保存
    DB-->>App: コミット完了

    Note over DB, Relay: ✅ ステップ2: 検知と送信
    Relay->>DB: 未送信行を監視/取得
    Relay->>Broker: イベント送信

    Note over Relay, DB: ✅ ステップ3: 完了マーク
    Broker-->>Relay: OK
    Relay->>DB: 送信済みに更新
```
