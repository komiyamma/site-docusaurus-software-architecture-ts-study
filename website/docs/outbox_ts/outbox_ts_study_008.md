...
![structure](./picture/outbox_ts_study_008_structure.png)
...
```mermaid
graph LR
    Dev[npm run dev] --> Watch[コード監視/実行⚡]
    Check[npm run typecheck] --> TSC[型チェック✅]
    Test[npm run test] --> Vitest[テスト実行🧪]
    Build[npm run build] --> Dist[dist生成🏗️]
```
